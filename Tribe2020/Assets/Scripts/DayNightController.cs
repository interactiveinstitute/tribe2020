...
public class DayNightController : MonoBehaviour {
	...
	void Initialize() {
		...
		if(GetComponent<Light>() != null) { lightIntensity = GetComponent<Light>().intensity; }
	}
	...
	/// Sets the currentPhase to Dawn, turning on the directional GetComponent<Light>(), if any.  
	public void SetDawn() {
		if(GetComponent<Light>() != null) { GetComponent<Light>().enabled = true; }
		currentPhase = DayPhase.Dawn;
	}

	/// Sets the currentPhase to Day, ensuring full day color ambient GetComponent<Light>(), and full  
	/// directional GetComponent<Light>() intensity, if any.  
	public void SetDay() {
		RenderSettings.ambientLight = fullLight;
		if(GetComponent<Light>() != null) { GetComponent<Light>().intensity = lightIntensity; }
	...
	/// Sets the currentPhase to Night, ensuring full night color ambient GetComponent<Light>(), and  
	/// turning off the directional GetComponent<Light>(), if any.  
	...
	/// If the currentPhase is dawn or dusk, this method adjusts the ambient GetComponent<Light>() color and direcitonal  
	/// GetComponent<Light>() intensity (if any) to a percentage of full dark or full GetComponent<Light>() as appropriate. Regardless  
	/// of currentPhase, the method also rotates the transform of this component, thereby rotating the  
	/// directional GetComponent<Light>(), if any.  
	private void UpdateDaylight() {
		if(currentPhase == DayPhase.Dawn) {
			...
			if(GetComponent<Light>() != null) { GetComponent<Light>().intensity = lightIntensity * (relativeTime / halfquarterDay); }
			...
			if(GetComponent<Light>() != null) { GetComponent<Light>().intensity = lightIntensity * ((halfquarterDay - relativeTime) / halfquarterDay); }
		}
