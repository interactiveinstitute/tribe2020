...
@implementation CustomVideoPlayer
{
    ...
    CustomCMVideoSampling             _videoSampling;
...
- (void)cleanupCVTextureCache
{
    ...
    CustomCMVideoSampling_Uninitialize(&_videoSampling);
}
...
- (int)curFrameTexture
{
    ...
    int curTex = CustomCMVideoSampling_LastSampledTexture(&_videoSampling);
    ...
    if(CMTimeCompare(_lastFrameTimestamp, _curFrameTimestamp) < 0)
    {
        ...
        curTex = CustomCMVideoSampling_SampleBuffer(&_videoSampling, _cmSampleBuffer, (int)_videoSize.width, (int)_videoSize.height);
    }
